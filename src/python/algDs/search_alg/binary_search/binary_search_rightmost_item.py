...
def binary_search_rightmost(arr: list[int], key: int) -> Optional[int]:
    """
    Perform a binary search to find the rightmost index of a key in a sorted list.
    ...
    Returns:
        The index of the rightmost occurrence of the key if found, otherwise None.
    """
    left = 0
    right = len(arr)
...
def binary_search_rightmost_exec_time(arr: list[int], key: int) -> Tuple[Optional[int], float]:
    """
    Perform a binary search to find the rightmost index of a key in a list and return the execution time.
...
def display_binary_search_rightmost(arr: list[int], key: int) -> None:
    """
    Display the result of the rightmost binary search operation.
    ...
    index, execution_time = binary_search_rightmost_exec_time(arr=arr, key=key)
    verify(index)
    print(f"[INFO] Execution time of finding the rightmost element using binary search: {execution_time*1000:.3f} milliseconds")
